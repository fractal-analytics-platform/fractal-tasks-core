...
def check_well_for_FOV_overlap(site_metadata, selected_well, always_plot=False):
    ...
    for line_1 in range(num_lines):
        ...
        for line_2 in range(line_1):
            ...
            overlap = is_overlapping_2D((min_x_1, min_y_1, max_x_1, max_y_1), (min_x_2, min_y_2, max_x_2, max_y_2))
            if overlap:
    ...
    if always_plot or (len(list_overlapping_FOVs) > 0):
        ...
        for line in range(num_lines):
            ...
            plt.text(0.5 * (min_x + max_x), 0.5 * (min_y + max_y), f"{line + 1}", ha="center", va="center", fontsize=14)
...
def run_overlap_check(site_metadata):
    ...
    for selected_well in wells:
        overlap_curr_well = check_well_for_FOV_overlap(site_metadata, selected_well=selected_well)
